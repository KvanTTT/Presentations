...
производить по типам токенов, а не по всему тексту.
</aside>
...
одинаковые и я, очевидно, либо опечатался, либо просто несу какую-то чушь.
...
к островным грамматикам - серым. Зеленым и красным цветом отмечаются токены
из входных данных. Библиотека токенизирует входные данные в точках инъекций и детектирует,
что они опасные, если количество результирующих токенов больше одного. На самом
деле в ней еще есть нюансы с авторизированными токенами, типами грамматик
и другими интересными вещами, но об этом и о принципах ее работы
расскажет Владимир на другом докладе :) <!-- Узнать как называется -->
</aside>
...
**Project: [DVWA](https://github.com/ethicalhack3r/DVWA); UI: [PT Application Inspector](https://www.ptsecurity.com/ru-ru/products/ai/)**
...
<aside class="notes">
Представляет собой совокупность графов UST, CFG и CPG.
</aside>
...
дополнительный этап компиляции, который также нужно реализовывать.
...
на основе грамматик ANTLR. Для тех, кто не знает, рассказываю: ANTLR - генератор
парсеров, принимающий на вход контекстно-свободную грамматику и возвращающий
...
DSL в расширенной форме Бэкуса-Наура (EBNF).
...
Кстати, в ANTLR таких возможностей тоже очень не хватает. Я как-то заводил задачу
по этому поводу: , но автор не
хочет ее реализовывать. Ввиду отсутствия такого универсального императивного
языка в ANTLR приходится либо делать грамматику зависимой от платформы, либо
как-то выкручиваться с помощью доступных средств, либо дублировать грамматики
для различных рантаймов. Понятно, что это не лучшие методы решения проблемы.
Дополнительный плюс: вставки на таком языке будут еще и проверяться на этапе
генерации парсера, а не на этапе компиляции или даже запуска кода
(для динамических языков). Впрочем, все эти мои соображения описаны там.
</aside>
